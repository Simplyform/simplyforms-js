...
# pnpm add @simplyforms/sdk · yarn add @simplyforms/sdk · bun add @simplyforms/sdk
```
...
> **Double-submit caveat:** `submit()` is **not idempotent**. A retried 5xx/network error *could*
> create a duplicate submission if the first request was actually received. 429s are always safe
> to retry (rejected, not processed). Set `retries: 0` if you need exactly-once.
...
```html
<script
  src="https://unpkg.com/@simplyforms/sdk@0.1.0/dist/browser/simplyforms.global.js"
  integrity="sha384-…"
